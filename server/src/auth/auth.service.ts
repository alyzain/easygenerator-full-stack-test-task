...
export class AuthService {
  ...
  async signUp(request: SignUpRequestDto): Promise<SignUpResponseDto> {
    ...
    if (existingUser) {
      this.logger.warn(
        `Sign-up attempt failed: User with email ${email} already exists.`,
      );
    ...
    try {
      ...
      this.logger.error(
        `Error hashing password for user ${email}: ${error.message}`,
        error.stack,
      );
  ...
  async signIn(request: SignInRequestDto): Promise<SignInResponseDto> {
    ...
    if (!user) {
      this.logger.warn(
        `Sign-in attempt failed: User with email ${email} not found.`,
      );
    ...
    if (!isPasswordValid) {
      this.logger.warn(
        `Sign-in attempt failed: Incorrect password for ${email}.`,
      );
      throw new UnauthorizedException('Invalid email or password');
    }
